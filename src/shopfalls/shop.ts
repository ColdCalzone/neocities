...
function handleNavigation(currentPage : number) {
    // Next page nav
    fetch("shopfalls.json", {cache: "no-cache"})
        .then((res : Response) => {
            return res.json();
        })
...
var ITEMS = fetch("items.json").then((res : Response) => { return res.json(); }).then((json : {[index: string] : ItemDescription}) => { return json; });
...
backgroundInv.src = "Inventory1.png";
const backgroundEq = new Image(400, 252);
backgroundEq.src = "Inventory2.png";
